...
import { TeachingAndLearning } from "@prisma/client";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { ColumnDef } from "@tanstack/react-table";
import axios, { AxiosResponse } from "axios";
import { MoreVertical } from "lucide-react";

export const columns: ColumnDef<TeachingAndLearning>[] = [
  {
    header: "Efforts for Extra Curriculum Activity",
    accessorKey: "efforts",
  },
  {
    header: "Actions",
...
const ActionsCell: React.FC<{ row: TeachingAndLearning }> = ({ row }) => {
  const queryClient = useQueryClient();

  const addProfile = async (id: string) => {
    const config = { headers: { "Content-Type": "application/json" } };
    const result: AxiosResponse = await axios.delete(
      `${process.env.NEXT_PUBLIC_ROUTE}/api/form/efforts-extra-curriculum/${id}`,
      config
    );
    return result.data;
  };
